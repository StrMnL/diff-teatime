...
我们希望刻画标量 $f(\mathbf{x})$ 关于向量 $\mathbf{x}$ **各个**分量变化的一个「整体趋势」，因此我们计算标量对向量每个分量的偏导数，再将这些偏导数按对应向量分量的顺序写成一个新的向量. 

由此定义 $f(\mathbf{x})$ 对 $\mathbf{x}$ 的导数为：

$$
\frac{\partial f(\mathbf{x})}{\partial \mathbf{x}} = \left[\frac{\partial f(\mathbf{x})}{\partial x_1}, \frac{\partial f(\mathbf{x})}{\partial x_2}, \cdots,\frac{\partial f(\mathbf{x})}{\partial x_n}\right].
$$

数学上遵循「**分子布局**」，将这个结果写成**行向量**. 

:::warning[注意]
但由于各种原因，机器学习中往往将标量对向量求导的结果当作列向量：
...
其中 $\nabla$ 是 $Nabla$ 算子，形式上 $\nabla_\mathbf{x}$ 可以写成列向量 $\left[\frac{\partial}{\partial x_1}, \frac{\partial}{\partial x_2}, \cdots,\frac{\partial}{\partial x_n}\right]^\top$. 

相对应地，这种写成列向量的写法称为**分母布局**. 

如果这里介绍的几种求导运算**统一**遵循分子布局或分母布局，就可以取得一些**形式上的统一**. 例如，后面介绍的[链式法则](#链式法则)便能够写成与一元标量函数一致的形式，只是将标量乘法变为矩阵乘法，且不同布局下矩阵乘法的顺序不同. 

不过在机器学习上，为了**工程上**的方便，会**混用两种布局**，例如下面向量对标量求导时遵循**分子布局**，而向量对向量求导又遵循**分母布局**. 

为了避免混淆，在本篇笔记的正文部分会使用分子布局. 在有必要时，我会特别说明结果的布局. 
:::

:::important[重要]
- 我们也称 $\nabla_{\mathbf{x}} f(\mathbf{x})$ 为多元函数的**梯度**. 梯度是一个关键的概念，对梯度所具有的意义的进一步介绍见[这里](/posts/d2l-preliminaries-calculus-3/). 
- 一般当我们说**梯度**时，会使用**分母布局**，即**列**向量的形式. 
:::
...
:::warning[注意]
这里机器学习的惯例与数学的惯例一致，使用的也是分子布局，而不是分母布局. 换句话说，机器学习习惯将向量对标量求导的结果和标量对向量求导的结果都当成列向量处理. 
:::
...
依次求 $f_1(\mathbf{x}),f_2(\mathbf{x}),\cdots,f_n(\mathbf{x})$ 对 $\mathbf{x}$ 的导数，遵循之前**分子布局**的形式，得到一系列行向量，然后写成一个 $n\times m$ 矩阵：
...
:::warning[注意]
机器学习则习惯用**分母布局**表示这个结果，将向量对向量求导结果写成 $Jacobi$ 矩阵的转置：
...
:::important[重要]
我们规定，**行**向量对**行**向量求导的结果是将它们**按列向量**求导结果**的转置**. 具体的结果取决于使用的布局. 
:::

## 常用向量导数结果

:::note[提示]
本部分的证明见[这里](/posts/d2l-preliminaries-calculus-2/). 
:::
...
1. 线性映射 $\mathbf{y=Ax}$ 关于 $\mathbf{x}$ 求导，在**分子布局**下，有：
...
:::warning[注意]
- 根据前面关于**行**向量对**行**向量求导的规定，若将这个线性映射写成行向量的形式 $\mathbf{y^\top = x^\top A^\top}$，则求导的结果也转置，即 $\frac{\partial \mathbf{y^\top}}{\partial \mathbf{x^\top}} = \mathbf{A}^\top. $
- 若按机器学习的习惯使用分母布局，求导结果应该转置. 也就是说，$\frac{\partial \mathbf{y}}{\partial \mathbf{x}} = \mathbf{A}^\top$，而 $\frac{\partial \mathbf{y^\top}}{\partial \mathbf{x^\top}} = \mathbf{A}^\top$. 
:::

2. 若 $m = n$，则二次型 $\mathbf{x}^\top \mathbf{A} \mathbf{x}$ 关于 $\mathbf{x}$ 的导数为**行向量**
...
:::warning[注意]
如果使用**分母布局**，或者采用**梯度**的说法，我们说二次型 $\mathbf{x}^\top \mathbf{A} \mathbf{x}$ 关于 $\mathbf{x}$ 的梯度为
$$
\nabla \mathbf{x}^\top \mathbf{A} \mathbf{x} = (\mathbf{A} + \mathbf{A}^\top) \mathbf{x}.
$$
:::

3. $\mathbf{x}$ 模长的平方关于 $\mathbf{x}$ 的导数为**行向量**
...
:::warning[注意]
如果使用**分母布局**，或者采用**梯度**的说法，我们说 $\mathbf{x}$ 模长的平方关于 $\mathbf{x}$ 的梯度为
$$
...
矩阵没有严格意义上的梯度，这里用 $\nabla$ 来表示标量 $\|\mathbf{X} \|_F^2$ 关于矩阵 $\mathbf{X}$ 求导，方式与**分母布局下**的**标量对向量求导**类似. 或者你也可以认为是「按元素求导」. 
:::
...
:::tip[提示]
这个求和式看上去可能有点费解. 不过这种写法不受布局的限制. 

在**分子布局**下，如果将上面的函数表示成 $y=f(\mathbf{u})$，$\mathbf{u}=g(\mathbf{x})$，其中 $\mathbf{u} = (u_1,u_2, \ldots ,u_m)$，$\mathbf{x} = (x_1, x_2, \dots, x_n)$ 的形式，就会发现上式可以写成 $\frac{\partial y}{\partial x_i} = \frac{\partial y}{\partial \mathbf{u}} \frac{\partial \mathbf{u}}{\partial x_i}$，其中 $\frac{\partial y}{\partial \mathbf{u}}$ 和 $\frac{\partial \mathbf{u}}{\partial x_i}$ 之间的运算是矩阵乘法. 可以验证这在形式上和结果上都是成立的. 

甚至，按这种写法，$\frac{\partial y}{\partial \mathbf{x}} = \frac{\partial y}{\partial \mathbf{u}} \frac{\partial \mathbf{u}}{\partial \mathbf{x}}$ 也是成立的. 推广到下面的多层复合中，这仍然是成立的. 

在分子布局或分母布局下，多元函数的链式法则不仅在形式上保持简洁，还与一元标量函数时的链式法则保持一致. 
:::
