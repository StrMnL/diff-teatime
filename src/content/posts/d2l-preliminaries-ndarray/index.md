...
- 对这些内容做了补充说明：
  - 用索引和切片的方式修改数组；
  - 在使用 `reshape()` 方法，以及进行 `tensor` 与 `ndarray` 间的转换时，内存分配上的特性。
- 增加了免责声明。
- 其它表述和排版微调。
...
- 对连续内存上的数组，`reshape()` 方法返回的是对原数组的一个引用，返回的数组与原数组仍指向同一块内存，与后面[类型转换](#类型转换)部分提到的 `tensor` 和 `ndarray` 之间的转换类似。如果你尝试更改返回后的数组，原数组也会发生变化。
...
例如，`arr.numel()` 会返回一个 Python 整数 `12`（不是 PyTorch 的标量哦）。
...
要将一个 `ndarray` 转换为 `tensor`，可以将它传入 `torch.from_numpy()` 方法或者 `torch.tensor()` 方法。不过这两种方法存在区别，详见下面「当心」部分的说明。

:::caution[当心]

- 当 `tensor` 位于 CPU 上时，其与 `ndarray` 互相转换前后的两个对象默认指向同一块内存，这会导致当你操作一个对象时，另一个对象的值也发生改变。
- 但是，如果 `tensor` 位于 GPU 上，用 `numpy()` 转换为 `ndarray` 会将它拷贝到 CPU 上，而**不会**发生上面的情况。如果使用 `torch.tensor()` 方法来将 `ndarray` 转换为 `tensor`，则总是会创建新的 `tensor`，也**不会**与原来的 `ndarray` 共享内存。
- 另外，`torch.from_numpy()` 无法将 CPU 上的 `ndarray` 转换为 GPU 上的 `tensor`。这种情况下应该使用 `torch.tensor()`。
